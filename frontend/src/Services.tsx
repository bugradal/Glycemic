...
export const pendingFoodList = () => {
    const headers = autControl()
    return axios.get(process.env.REACT_APP_BASE_URL+"/foods/adminPendingList",{
        headers: headers
    })
}
...
// admin publish pending foods
export const adminPublishPendingFoods = (item: ResultFoods) => {
    const headers = autControl()
    const obj = {
        "gid": item.gid,
        "cid": item.cid,
